...
class SyncZchunkRepoSkipTestCase(unittest.TestCase):
    ...
    1. Clean all existing orphans to prevent false failures.
    2. Create a repo point to a ``zchunk`` feed. Synch and publish the repo.
    3. Assert no published with the ``.zck`` extension.
    4. Assert all other units with the repo exist.
    ...
    def test_zchunk_sync(self):
        ...
        # Verify other content units exist

        # retrieving the published repo
        xml_element = get_repodata_repomd_xml(cfg, repo['distributors'])
        xpath = (
            '{{{namespace}}}data/{{{namespace}}}checksum'.format(
                namespace=RPM_NAMESPACES['metadata/repo']
            )
