...
from pulp_smash import api, cli, utils, selectors, config
...
class InstallDistributorTestCase(unittest.TestCase):
    ...
    def test_all(self):
        ...
        if (not selectors.bug_is_fixed(3314, cfg.pulp_version) and
                os_is_f27(cfg)):
        ...
        upload_import_unit(
            cfg, unit, {'unit_type_id': 'puppet_module'}, repo)
...
class InstallDistributorThrowsOnErrorTestCase(unittest.TestCase):
    """Test Puppet install distributor."""
    ...
    def test_all(self):
        ...
        """
        cfg = config.get_config()
        if not selectors.bug_is_fixed(1237, cfg.pulp_version):
